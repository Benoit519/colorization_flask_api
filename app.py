...
def colorize():
    ...
    #gray = gray/maximum_img
    gray = (gray-max_divided)/max_divided
    ab_img = model.predict(gray)
    ab_img = 127.5*ab_img +127.5
    gray= 127.5*gray + 127.5
    gray = np.reshape(gray,(224,224,1))
    ab_img = np.reshape(ab_img,(224,224,2))
    
   
    img = np.concatenate([gray,ab_img],2)
